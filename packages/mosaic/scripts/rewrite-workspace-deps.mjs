...
for (const section of sections) {
	...
	for (const name of Object.keys(deps)) {
		...
		const siblingPkgPath = path.join(
			monorepoPackagesDir,
			bareName,
			'package.json',
		);
		...
		if (!sibling.version) {
			console.error(
				`[rewrite-workspace-deps] sibling ${name} has no version field`,
			);
			process.exit(1);
		}
...
if (rewrites.length === 0) {
	console.log(
		'[rewrite-workspace-deps] no workspace: refs found — nothing to rewrite',
	);
	process.exit(0);
}
...
console.log(
	`[rewrite-workspace-deps] rewrote ${rewrites.length} workspace: ref(s) for publish:`,
);
