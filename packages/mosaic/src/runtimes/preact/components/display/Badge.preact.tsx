...
function BadgeInner({ label, variant, dot, count, locale }: BadgeProps) {
  ...
  return (
    <span
      ...
      lang={locale}
      class={`inline-flex items-center gap-1 rounded-full px-2.5 py-0.5 text-xs font-medium ${variantClasses[variant]}`}
    >
      {dot && <span aria-hidden="true" class={`h-1.5 w-1.5 rounded-full ${dotClasses[variant]}`} />}
      {label}
      {count !== undefined && (
        <span aria-hidden="true" class="font-semibold tabular-nums">
          {count}
        </span>
      )}
    </span>
...
/**
 * Badge — cross-runtime generic visual label primitive (Preact 10 implementation).
 *
