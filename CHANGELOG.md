...
### Added — Wave 2 T13 — `Select` field primitive (forms)
- **New field primitive** `Select` in `forms` category. Single-select dropdown implementing the ARIA combobox+listbox pattern (APG 2024) with optional in-popup search.
- **API**: `name` (required, non-empty), `label` (required), `options: Array<{value, label, disabled?}>` (min 1 item), `placeholder?`, `disabled?` (default false), `searchable?` (default false), `className?`. Locale: `"en" | "fr"` (default `"en"`).
- **Shared pure logic** (`Select.logic.ts`): `filterOptions`, `indexOfValue`, `findFirstEnabledIndex`, `findLastEnabledIndex`, `findNextEnabledIndex`, `findByTypeAhead`, `resolveSelectedLabel`, plus ID helpers (`getTriggerId`, `getListboxId`, `getOptionId`), Tailwind class generators (`getTriggerClasses`, `getListboxClasses`, `getOptionClasses`, `getSearchInputClasses`). Runtime-agnostic — both runtimes share by reference (Standard §18 contract).
- **WCAG-AA strict**: `role=combobox` on trigger with `aria-haspopup=listbox`, `aria-expanded`, `aria-controls`, `aria-labelledby`. `role=listbox` on popup. `role=option` + `aria-selected` on each item. `aria-activedescendant` tracks highlighted option without moving DOM focus. `aria-disabled` on disabled options. Focus management: popup close restores focus to trigger.
- **Keyboard navigation**: ArrowDown (next enabled, wraps) · ArrowUp (prev enabled, wraps) · Home (first enabled) · End (last enabled) · Enter (commit active option, close) · Escape (close, restore) · Printable type-ahead (500ms buffer, jumps to first matching label). Closed: Enter/Space/ArrowDown/ArrowUp opens popup.
- **Search mode**: when `searchable=true`, an `<input aria-label="Search options">` autofocuses inside the popup. Query filters by case-insensitive substring on `label`. Empty query shows all. "No options" message when filtered list is empty.
- **Disabled option**: rendered in listbox but skipped by all navigation helpers (ArrowUp/Down/type-ahead) and `mouseDown` commit is a no-op.
- **Cross-runtime**: React 19 at `@vantageos/mosaic/react/forms`, Preact 10 at `@vantageos/mosaic/preact/forms`, back-compat re-export at `@vantageos/mosaic/forms`.
- **Registry**: `Select` entry under `forms` (sizeLimit 15KB, WCAG-AA, i18nKeys: `Select.placeholder`, `Select.search.placeholder`, `Select.empty`).
- **Storybook**: 4 stories — `Default (simple dropdown)`, `Searchable (with search input)`, `With disabled option`, `Disabled (entire control)`. Bilingual labels (FR + EN) in all stories.
- **Tests (TDD RED→GREEN)**: React test file covers Zod schema (8 cases) + SelectOptionSchema (4 cases), all pure logic helpers (ID helpers, filterOptions, indexOfValue, navigation, type-ahead, resolveSelectedLabel), runtime tests: trigger render, label, placeholder, aria-expanded toggle, options render, disabled aria-disabled, click-to-select + close, aria-selected on selected, Escape/ArrowDown keyboard, Home/End nav, aria-activedescendant tracking (open/closed), type-ahead jump, disabled control no-open, searchable renders search input, search filter, "No options" empty state, aria-controls/aria-haspopup wiring, onBlur timing (no error on change, error appears after blur — T10 lesson #4/#5). Preact parity smoke verifies module export + all shared logic helpers by reference.
